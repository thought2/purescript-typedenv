  ...
  describe "fromEnv" do
    ...
    it "reads the requested environment" do
      let
        ...
        actual = fromEnv (Proxy :: Proxy (b :: String <: "B", c :: String <: "C")) env
    ...
    it "indicates when a lookup has failed" do
      let
        ...
        actual = fromEnv (Proxy :: Proxy (message :: String <: "MESSAGE")) env
      actual `shouldEqual` expected
    ...
    it "indicates when parsing a value has failed" do
      let
        ...
        actual = fromEnv (Proxy :: Proxy (debug :: Boolean <: "DEBUG")) env
      actual `shouldEqual` expected
    ...
    it "parses boolean values" do
      traverse_
        (\({ given, expected }) ->
          shouldEqual
            (fromEnv (Proxy :: Proxy (actual :: Boolean <: "A")) (FO.fromHomogeneous { "A": given }))
    ...
    it "parses integer values" do
      let
        ...
        actual = fromEnv (Proxy :: Proxy (value :: Int <: "VALUE")) env
      actual `shouldEqual` expected
    ...
    it "parses character values" do
      let
        ...
        actual = fromEnv (Proxy :: Proxy (value :: Char <: "VALUE")) env
      actual `shouldEqual` expected
    ...
    it "parses number values" do
      let
        ...
        actual = fromEnv (Proxy :: Proxy (value :: Number <: "VALUE")) env
      actual `shouldEqual` expected
    ...
    it "parses optional values" do
      let
        ...
        actual = fromEnv (Proxy :: Proxy (value :: Maybe String <: "VALUE")) env
      actual `shouldEqual` expected

    it "allows optional values to be absent" do
      let
        expected = Right { value: Nothing }
        actual = fromEnv (Proxy :: Proxy (value :: Maybe String <: "VALUE")) FO.empty
      actual `shouldEqual` expected
